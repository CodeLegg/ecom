...
def get_ordered_products_by_collection(collection):
    return (
        Product.objects.filter(
            in_collections__collection__in=collection.get_descendants(
                include_self=True
            ),
            is_active=True,
        )
        .order_by("in_collections__order")
        .distinct()
    )
...
def home(request):
    ...
    stickers_collection = get_object_or_404(
        Collection, name="Stickers, Labels, Design & Print"
    )
    ...
    products = (
        Product.objects.filter(
            in_collections__collection__in=stickers_collection.get_descendants(
                include_self=True
            )
...
def sticker_collections_product_list(request, slug):
    ...
    if collection.parent != stickers_collection:
        messages.error(
            request,
            "That Collection is not part of the 'Stickers, Labels, Design & Print' collection.",
        )
        return redirect("sticker_collections")
...
def sticker_sub_collections_list(request, slug):
    ...
    return render(
        request,
        "sub_collections_list.html",
        {"collection": collection, "sub_collections": sub_collections},
    )
...
def sticker_sub_collections_product_list(request, slug):
    ...
    return render(
        request,
        "sub_collections_product_list.html",
        {"sub_collection": sub_collection, "products": products},
    )
...
def cbd_collections_product_list(request, slug):
    ...
    if collection.parent != cbd_packaging:
        messages.error(
            request, "That Collection is not part of the 'CBD Packaging' collection."
        )
        return redirect("cbd_collections")
...
from .forms import SignUpForm
from .emails import send_welcome_email  # Import the send_welcome_email function
...
def register_user(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            username = form.cleaned_data.get("username")  # Assuming your form collects a username
            ...
            if user is not None:
                ...
                # Send welcome email
                send_welcome_email(user_email=email, user_name=username)
